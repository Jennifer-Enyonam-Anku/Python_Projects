...
st.markdown("""
    <style>
        ...
        /* Remove white border from active menu item */
        div[data-testid="stSidebar"] .nav-link:focus,
        div[data-testid="stSidebar"] .nav-link:focus-visible,
...
# ------------------------------------------------
if selected == "Home":
    st.title("\U0001F3E0 Welcome to the HR Exit Predictor")
    st.markdown("""
...
# ------------------------------------------------
    ...
    st.title("\U0001F9E0 Employee Exit Probability Predictor")
    ...
    if st.button("Predict Exit Probability"):
        ...
        st.markdown("### \U0001F4A1 HR Recommendation")
        if prob < 30:
            st.info("\U0001F7E2 **Low Risk:** The employee is likely to stay. Continue monitoring and provide regular support.")
        elif 30 <= prob <= 70:
            st.warning("\U0001F7E1 **Moderate Risk:** Engage the employee. Consider offering professional development, mentorship, or workload reviews.")
        else:
            st.error("\U0001F534 **High Risk:** Take immediate action. Consider a one-on-one conversation, career growth incentives, or team reassignment.")

# ------------------------------------------------
# VIEW DATA TAB
# ------------------------------------------------
elif selected == "View Data":
    st.title("\U0001F4C1 View Employee Records")
    ...
    try:
        ...
        with st.expander("\U0001F4C4 Preview DataFrame"):
            st.dataframe(df.style.set_properties(**{'background-color': '#F0F4FF'}, subset=df.columns))

        with st.expander("\U0001F9BE Data Summary"):
            st.write(df.describe(include='all').T)

        st.markdown("### \U0001F39B️ Filter Data")
        ...
        st.markdown("### \U0001F4CA Insights")
